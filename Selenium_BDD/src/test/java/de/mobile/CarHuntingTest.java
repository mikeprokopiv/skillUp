...
public class CarHuntingTest {

    @Test
    @DisplayName("Verify car with lowest price was parked")
...
//        driver.manage().timeouts().implicitlyWait(1, TimeUnit.SECONDS);
        ...
        List<WebElement> searchedResultCarsPrices = driver.findElements(By.xpath("//span[@class='h3 u-block']"));
        int firstAdvertisedCarPrice = Integer.parseInt(searchedResultCarsPrices.get(1).getText().replace(",", "").substring(1, 6));
        ...
        if (firstAdvertisedCarPrice <= secondAdvertisedCarPrice) {
            boolean priceLowestFirstSorting = secondAdvertisedCarPrice <= thirdAdvertisedCarPrice;
            Assertions.assertTrue(priceLowestFirstSorting);
        }
        ...
        Assertions.assertEquals(firstAdvertisedCarPrice, priceParkedCar);
